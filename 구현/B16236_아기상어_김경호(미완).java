...
//				if(fish>0) fishes[i*N+j] = true;
	...
	public static int[] bfs(int r, int c) {
		...
		while(!queue.isEmpty()) {
			...
			int nextC = c;
			int i=0;
			for(int nextR= r+ cur[2]+1 ; nextR>=r-(cur[2]+1);nextR--){
				if(!(isOut(nextR,nextC-i)|| visited[nextR][nextC-i]|| board[nextR][nextC-i] > state)) {
					if(isOut(nextR,nextC+i)|| visited[nextR][nextC+i]|| board[nextR][nextC+i] > state) {
						queue.add(new int[] {nextR,nextC-i,cur[2]+1});
						i++;
						continue;
					}
					queue.add(new int[] {nextR,nextC+i,cur[2]+1});
...
//				queue.add(new int[] {nextR,nextC,cur[2]+1});
	...
	static class Fish implements Comparable<Fish>{
		...
		public int compareTo(Fish o) {
			...
			return this.size - o.size == 0? 
					(this.r-o.r==0 ? this.c - o.c:this.r-o.r) : this.size - o.size;
		}
